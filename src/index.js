...
async function runAutoMigrations() {
  try {
    ...
    let widenOk = 0;
    for (const sql of widenStatements) {
      try { await pool.query(sql); widenOk++; } catch (e) { /* column already TEXT or does not exist */ }
    }
    logger.info(`Widen columns: ${widenOk}/${widenStatements.length} applied`);
