...
async function saveListing(listing) {
  try {
    ...
    if (!match) {
      logger.debug(`[Dira] No complex match for: ${listing.address}, ${listing.city} — skipping`);
      return 'skipped';
    }
    ...
    logger.debug(`[Dira] Saved listing for complex ${match.complexName}: ${listing.address}`);
    return 'inserted';
  } catch (err) {
