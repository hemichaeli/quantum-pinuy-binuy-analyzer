/**
 * Komo (komo.co.il) Scraper - Issue #4 P2
 * Scrapes property listings from komo.co.il
 * Stores in `listings` table (source='komo')
...
// Try Komo direct API
async function queryKomoDirect(city, limit = 50) {
  try {
    // Komo uses a search API
    const response = await axios.get('https://www.komo.co.il/api/search', {
      params: {
        city,
        dealType: 'sale',
    ...
    if (response.data?.items?.length > 0) {
      return response.data.items.map(item => parseKomoItem(item, city));
    }
    // Try alternate endpoint
    const r2 = await axios.get('https://www.komo.co.il/nadlan/forsale', {
      params: { city, page: 1 },
      headers: HEADERS,
      timeout: 15000
    });
    if (r2.data?.properties?.length > 0) {
      return r2.data.properties.map(item => parseKomoItem(item, city));
    }
    return null;
  } catch (err) {
    logger.debug(`[Komo] Direct API failed for ${city}: ${err.message}`);
    return null;
  }
}

function parseKomoItem(item, defaultCity) {
...
// Perplexity fallback for Komo
async function queryKomoPerplexity(city) {
  const apiKey = process.env.PERPLEXITY_API_KEY;
  if (!apiKey) return [];

  const prompt = `חפש מודעות מכירת דירות פעילות באתר komo.co.il בעיר ${city}.
החזר JSON בלבד:
{
  "listings": [
    {
  ...
  try {
    ...
    logger.debug(`[Komo] Perplexity failed for ${city}: ${err.message}`);
    return [];
  }
}
...
async function saveListing(listing) {
  try {
    ...
    logger.warn(`[Komo] Save error: ${err.message}`);
    return 'error';
  }
}

async function scanCity(city) {
  logger.info(`[Komo] Scanning ${city}...`);
  let rawListings = await queryKomoDirect(city);
  let source = 'komo_api';
  if (!rawListings || rawListings.length === 0) {
    rawListings = await queryKomoPerplexity(city);
    source = 'perplexity';
  }
  if (!rawListings || rawListings.length === 0) {
    logger.debug(`[Komo] No listings found for ${city}`);
  ...
  logger.info(`[Komo] ${city}: ${inserted} new, ${updated} updated (via ${source})`);
...
async function scanAll(options = {}) {
  ...
  logger.info(`[Komo] Starting scan of ${citiesToScan.length} cities`);
  ...
  for (let i = 0; i < citiesToScan.length; i++) {
    try {
      ...
      logger.error(`[Komo] Error for ${citiesToScan[i]}: ${err.message}`);
  ...
  logger.info('[Komo] Scan complete:', { totalInserted, totalUpdated });
